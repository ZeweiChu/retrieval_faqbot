...
class FAQProcessor(DataProcessor):
    ...
    def get_candidates(self, file_dir):
        train_df = pd.read_csv(file_dir)#.iloc[:100]
...
def evaluate(args, model, eval_dataset): 
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            sequence_output = outputs[0].cpu().data.mean(1)
            sequence_outputs.append(sequence_output)
            # pooled_output: [CLS] token的表示经过一个线性变换和一个tanh层之后的结果
...
def main():
    ...
    if not os.path.exists(args.data_dir+ "_bert_embeddings.pkl"):
        ...
        eval_dataset, candidate_title, candidate_reply = load_examples(args, args.task_name, tokenizer)
