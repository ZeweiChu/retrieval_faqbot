...
#    code.interact(local=locals())
    ...
    return pooled_outputs, sequence_outputs
...
def main():
    ...
    pooled_outputs, sequence_outputs = evaluate(args, model, eval_dataset)
    # candidate_embeddings = torch.cat([o.cpu().data for o in pooled_outputs]).numpy()
    candidate_embeddings = torch.cat([o.cpu().data.mean(1) for o in sequence_outputs]).numpy()
    ...
    while True:
        ...
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids)
        pooled_outputs, sequence_outputs = evaluate(args, model, dataset)
        title_embedding = torch.cat([o.cpu().data.mean(1) for o in sequence_outputs]).numpy()
